    ...
    <div class="container">
        ...
        <div class="menu-item">
            ...
            <form action="index.php" method="get" class="form-inline justify-content-center mb-4">
                <div class="form-group mx-sm-3">
                    <label for="search" class="sr-only">Search by Customer Name</label>
                    <input type="text" class="form-control" id="search" name="search" placeholder="Search by Customer Name">
                </div>
            ...
            $sql = "SELECT invoice_id, customer_name FROM invoices";

            if (!empty($search)) {
                $sql_total .= " WHERE customer_name LIKE '%$search%'";
                $sql .= " WHERE customer_name LIKE '%$search%'";
            }

            $sql .= " ORDER BY invoice_id LIMIT $limit OFFSET $offset";
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<a href="../index.php?invoice_id=' . $row['invoice_id'] . '" class="btn btn-sm btn-info">View</a> ';
                ...
                if ($page > 1) {
                    echo '<li class="page-item"><a class="page-link" href="index.php?page=' . ($page - 1) . '&search=' . urlencode($search) . '">Previous</a></li>';
                }
                for ($i = 1; $i <= $total_pages; $i++) {
                    echo '<li class="page-item ' . ($i == $page ? 'active' : '') . '"><a class="page-link" href="index.php?page=' . $i . '&search=' . urlencode($search) . '">' . $i . '</a></li>';
                }
                if ($page < $total_pages) {
                    echo '<li class="page-item"><a class="page-link" href="index.php?page=' . ($page + 1) . '&search=' . urlencode($search) . '">Next</a></li>';
                }
                echo '</ul></nav>';
            } else {
